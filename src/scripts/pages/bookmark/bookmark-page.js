...
class BookmarkPage {
  ...
  renderSavedStories(stories) {
    ...
    if (stories.length === 0) {
      container.innerHTML = '<p style="text-align:center;">Belum ada cerita yang disimpan.</p>';
      return;
    }
      ...
      button.addEventListener("click", async (e) => {
        ...
        if (confirm("Apakah Anda yakin ingin menghapus cerita ini?")) {
          try {
            ...
            const container = document.getElementById("saved-story-list");
            if (container && container.querySelectorAll(".story").length === 0) {
              container.innerHTML = '<p class="empty-message">Belum ada cerita yang disimpan.</p>';
            }
          } catch (error) {
